...
const Image = ({ img, onClick, canDelete, onDelete, loading, onEdit }) => (
  <div className="relative select-none group">
    <img
      ...
      // Uncomment if click behavior is needed
      // onClick={onClick}
      onDoubleClick={onEdit}
    />
    {canDelete && (
      <div
        className="sm:w-7 sm:h-7 w-6 h-6 rounded-full bg-secondary flex justify-center items-center bg-opacity-50 absolute sm:bottom-2 bottom-1 ring-2 right-1 sm:right-2 cursor-pointer opacity-0 scale-0 group-hover:opacity-100 group-hover:scale-100 transition-all duration-500"
        onClick={onDelete}
      >
        {loading ? (
    ...
    <div
      className="sm:w-7 sm:h-7 w-6 h-6 rounded-full bg-secondary flex justify-center items-center bg-opacity-50 absolute sm:bottom-2 bottom-1 ring-2 left-1 sm:left-2 cursor-pointer opacity-0 scale-0 group-hover:opacity-100 group-hover:scale-100 transition-all duration-500"
      onClick={onEdit}
    >
