  ...
  useEffect(() => {
    if (download || currentPage === "crop") {
      ...
      image.onload = () => {
        ...
        const borderRadius = menuItemOptions.find(
          (item) => item.property === "borderRadius"
        )?.value || 0;
  
        const rotate = menuItemOptions.find(
          (item) => item.property === "rotate"
        )?.value || 0;
        ...
        ctx.beginPath();
        const radius = Math.min(canvas.width, canvas.height) * (borderRadius / 100); // Border radius as percentage
        ctx.moveTo(radius, 0);
